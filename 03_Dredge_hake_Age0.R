...
setwd(DredgeFile)
####################################################################
##### BRING IN HAKE DATA - LATE LARVAL STAGE ####################
####################################################################
...
roms = data.frame(read.csv(paste0(MainFile,'/01_ROMS_Prelim/Data_ROMS.for.hake.analysis.age0.csv'), header=TRUE))
...
#### Set up Formula #### (aic - aicq) dAIC < 2 - none for Age-O stage
form_dredge = as.formula(paste('dev ~ ', ROMS, sep=''))
# 
form_dredge

saveRDS(form_dredge, "Formula_for_Dredge.hake_age0.rds")

#### Prep & combine data ####
# data_1 = # combine roms data and resids
data_1 = merge(fish,roms, 'year')
data_1 = na.omit(data_1)
write.table(data_1, 'Analyzed.data.hake_age0.csv',sep=',',col.names = TRUE, row.names = FALSE)
...
mtable = dredge(fit, rank = AICc, m.lim = c(NA,5),
                subset= # block correlated terms from the model > 0.70
                  ...
                  !(CSTpjuv.age.0.n && ASTpjuv.age.0 ) &&  # 0.82
                  
                  !(DDpjuv.age.0 && SSH.ms.c) &&  # 0.80
                  !(DDpjuv.age.0 && SSH.ms.s) &&  # 0.83  
              
                
                
                  !(SSH.ms.c && SSH.ms.s) &&  # 0.97
                  !(SSH.ms.c && UWpjuv.age.0.bn) &&  # -0.70
                  
                  
                  !(UWpjuv.age.0.bn && UWpjuv.age.0.bs) &&  # 0.83
                  !(UWpjuv.age.0.bn && UWpjuv.age.0.cn) &&  # 0.76
                 
                  !(UWpjuv.age.0.bs && UWpjuv.age.0.cs) &&  # 0.72 
                  
                  !(UWpjuv.age.0.cn && UWpjuv.age.0.cs), # 0.85
...
saveRDS(mtable, file='Table_model.fits.hake_age0.rds')
write.csv(mtable2,"R_Table_Delta2.hake_age0.csv")



mtable = readRDS('Table_model.fits.hake_age0.rds')
mtable2 = subset(mtable, delta < 2)
mtable2
