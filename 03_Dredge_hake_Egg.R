...
roms = data.frame(read.csv(paste0(MainFile,'/01_ROMS_Prelim/Data_ROMS.for.hake.analysis.egg.csv'), header=TRUE))
...
#### Set up Formula #### no variables have AIC < 2 (AIC - AICq) - none for egg stage
form_dredge = as.formula(paste('dev ~ ', ROMS, sep=''))
# 
form_dredge

saveRDS(form_dredge, "Formula_for_Dredge.hake_egg.rds")

#### Prep & combine data ####
# data_1 = # combine roms data and resids
data_1 = merge(fish,roms, 'year')
data_1 = na.omit(data_1)
write.table(data_1, 'Analyzed.data.hake_egg.csv',sep=',',col.names = TRUE, row.names = FALSE)
...
mtable = dredge(fit, rank = AICc, m.lim = c(NA,5),
                subset= # block correlated terms from the model > 0.70
                  !(DDegg.s && SSH.ja.s) &&  # 0.75
                  !(DDegg.s && SSH.ja.c) &&  # 0.80
...
saveRDS(mtable, file='Table_model.fits.hake_egg.rds')
write.csv(mtable2,"R_Table_Delta2.hake_egg.csv")



mtable = readRDS('Table_model.fits.hake_egg.rds')
mtable2 = subset(mtable, delta < 2)
mtable2
