...
roms = data.frame(read.csv(paste0(MainFile,'/01_ROMS_Prelim/Data_ROMS.for.hake.analysis.yolk.csv'), header=TRUE))
...
#### Set up Formula #### no variables have AIC < 2 (AIC - AICq) - none for this stage
form_dredge = as.formula(paste('dev ~ ', ROMS, sep=''))
# 
form_dredge

saveRDS(form_dredge, "Formula_for_Dredge.hake_yolk.rds")

#### Prep & combine data ####
# data_1 = # combine roms data and resids
data_1 = merge(fish,roms, 'year')
data_1 = na.omit(data_1)
write.table(data_1, 'Analyzed.data.hake_yolk.csv',sep=',',col.names = TRUE, row.names = FALSE)
...
mtable = dredge(fit, rank = AICc, m.lim = c(NA,5),
                subset= # block correlated terms from the model > 0.70
                  !(DDyolk && SSH.ja.c) &&  # 0.79
                  !(DDyolk && SSH.ja.s) &&  # 0.75
...
saveRDS(mtable, file='Table_model.fits.hake_yolk.rds')
write.csv(mtable2,"R_Table_Delta2.hake_yolk.csv")



mtable = readRDS('Table_model.fits.hake_yolk.rds')
mtable2 = subset(mtable, delta < 2)
mtable2
