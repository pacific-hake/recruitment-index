...
setwd(paste0(MainFile,'/04_Model.Testing/03_Resample_Recruitment_w_error'))
...
for(k in 1:1000) {# refit model 1000 times
  # create new recruitment time series
  print(paste("k = ", k))
  
  # swap resids or log resids.  set in RUN.ALL file
  ...
  write.table(results, 'R_boot_sd_hake_all_stages_ROMS_PPC.csv', sep=',',col.names = TRUE, row.names = FALSE)
  ...
  x[1] <- "fitted model"
  write.csv(boot.stats2,'R_boot.stats_sd_hake_all_stages_ROMS_PPC.csv')
boot.stats2
