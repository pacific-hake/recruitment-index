...
setwd(paste0(MainFile,'/04_Model.Testing/01a_Bootstrap_Random_Data'))
library(MuMIn)
...
for(k in 1:1000) {# refit model 1000 times
  ...
  write.csv(results, "R_boot_random_deviations_hake_all_stages_ROMS_PPC.csv")
...
# quantile function
  ...
  boot.results
  write.csv(boot.results,'Stats_boot_random_deviations_hake_all_stages_ROMS_PPC.csv')
