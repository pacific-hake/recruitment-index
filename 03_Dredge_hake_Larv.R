...
setwd(DredgeFile)
####################################################################
##### BRING IN HAKE DATA - FIRST-FEEDING LARVAL STAGE ####################
####################################################################
...
roms = data.frame(read.csv(paste0(MainFile,'/01_ROMS_Prelim/Data_ROMS.for.hake.analysis.larv.csv'), header=TRUE))
...
form_dredge = as.formula(paste('dev ~ I(CSTlarv.s^2) +', ROMS, sep=''))
# 
form_dredge

saveRDS(form_dredge, "Formula_for_Dredge.hake_larv.rds")

#### Prep & combine data ####
# data_1 = # combine roms data and resids
data_1 = merge(fish,roms, 'year')
data_1 = na.omit(data_1)
write.table(data_1, 'Analyzed.data.hake_larv.csv',sep=',',col.names = TRUE, row.names = FALSE)
...
mtable = dredge(fit, rank = AICc, m.lim = c(NA,5),
                subset= # block correlated terms from the model > 0.70
                  ...
                  !(UWlarv.bn && UWlarv.bs) &&  # 0.90
                  !(UWlarv.bn && UWlarv.cn) &&  # 0.85
                  
                  !(UWlarv.bs && UWlarv.cn) &&  # 0.79
                  !(UWlarv.bs && UWlarv.cs) &&  # 0.81
                  
                  !(UWlarv.cn && UWlarv.cs) &&  # 0.77
                  
                  !(DDlarv && SSH.ja.c) &&  # 0.82
                  !(DDlarv && SSH.ja.s) &&  # 0.80
                  !(DDlarv && EKE.ja.s) &&  # 0.71
                  
                  !(SSH.ja.c && SSH.ja.s) &&  # 0.92
                  !(SSH.ja.c && UWlarv.bn) &&  # -0.79
                  
                  !(SSH.ja.s && UWlarv.bn) &&  # -0.71
...
saveRDS(mtable, file='Table_model.fits.hake_larv.rds')
write.csv(mtable2,"R_Table_Delta2.hake_larv.csv")



mtable = readRDS('Table_model.fits.hake_larv.rds')
mtable2 = subset(mtable, delta < 2)
mtable2
